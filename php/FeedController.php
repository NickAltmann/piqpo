...
{
	static function create($name, $pause, $pollFrequency, $maxSlides, $templateFile, $definitionFile, $parameters)
	{
		...
		// TODO
		if (strlen($definitionFile) == 0)
		{
			$returnValue->addError("Definition file must supplied.");
		}
		...
		{
			$returnValue->addError("Template file must supplied.");
		}
		...
		if ($returnValue->success())
		{
			...
			$feedId = Feed::create($definitionFile, $name, $pause, $pollFrequency, $templateFile, $maxSlides);
    ...
    function modify($name, $pause, $pollFrequency, $maxSlides, $templateFile, $definitionFile, $parameters)
	{
		...
		$this->_feed->update($definitionFile, $name, $pause, $pollFrequency, $templateFile, $maxSlides); 
	...
	{	
		...
		$feedFilename = $feedDefinitionManager->formFullFilename( $this->feedDefinitionFile() );
